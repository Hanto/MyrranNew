...
public class DaD //extends DragAndDrop
{
            ...
            public void drag (InputEvent event, float x, float y, int pointer) {
                ...
                if (hit != null) {
                    for (int i = 0, n = targets.size; i < n; i++) {
                        Target hitTarget = targets.get(i);
                        if (!hitTarget.actor.isAscendantOf(hit)) continue;
                        newTarget = hitTarget;
                        hitTarget.actor.stageToLocalCoordinates(tmpVector.set(stageX, stageY));
                        break;
                    }
                }
